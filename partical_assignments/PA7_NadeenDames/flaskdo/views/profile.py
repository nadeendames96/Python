    ...
    try:
        ...
        if user:
            if request.method == 'GET':
                ...
                picture.save(secure_filename(picture.filename))
                address = request.form['address']

                db.execute(
                    "UPDATE User SET email=?, firstname=?, lastname=?, bod=?, picture=?, address=? WHERE id=?;", (email, firstname, lastname, bod, picture, address, session['uid'],)).fetchone()
