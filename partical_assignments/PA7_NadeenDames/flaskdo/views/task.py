...
def add_task():
    if request.method == 'GET':
        ...
        tags=request.form['task-tags']
        due=request.form['task-due']
        ...
        try:
            # execute the SQL query
            db.execute(
                "INSERT INTO Task (title, description,tags,due,priority, task_list_id) VALUES (?, ?, ?, ?,?,?);", (title, description,tags,due,priority, task_list_id))
    ...
    try:
        ...
        if task:
            if request.method == 'GET':
                ...
                # update task in DB
                tasks = db.execute('UPDATE Task SET title=?, description=?, priority=?,due=?,tags=?,task_list_id=? WHERE id = ?', (
                    title, description, priority,due,tags,task_list_id, task_id,))
